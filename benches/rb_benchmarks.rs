...
use generic_ringbuffers::{
    ConstMultiThreadedRingBuffer, LFShardedMultiThreadedRingBuffer, MultiThreadedRingBuffer,
    ShardedMultiThreadedRingBuffer,
};
use std::{
...
fn benchmark_ringbuffer(capacity: usize) {
    let cmtrb = Arc::new(Mutex::new(AllocRingBuffer::new(capacity)));
    ...
    for _ in 0..MAX_THREADS {
        ...
        let handler: thread::JoinHandle<usize> = thread::spawn(move || {
            ...
            for _i in 0..capacity {
                let item: Option<usize> = cmtrb.lock().unwrap().dequeue();
                match item {
                    Some(_) => counter += 1,
                    None => break,
                }
...
fn rb_benchmark(c: &mut Criterion) {
    ...
    // The const ring buffer allocates on the stack, so it can't handle 100000 or more
    c.bench_with_input(
        BenchmarkId::new("alloc_ring_buffer", CAPACITY),
        &CAPACITY,
        |b, &s| {
            // Insert a call to `to_async` to convert the bencher to async mode.
            // The timing loops are the same as with the normal bencher.
